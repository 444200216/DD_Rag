import type {
  ...
  PendingInvitationItem, OwnerJoinRequestItem, UserLookupResult,
} from '@/types/ddrag'
...
export async function createInvitation(groupId: number, inviteeUserId: number): Promise<number> {
  return ddragPost<number>(`/groups/${groupId}/invitations`, { inviteeUserId })
}

export async function lookupUserByCode(userCode: string): Promise<UserLookupResult> {
  return ddragGet<UserLookupResult>('/account/lookup-user', { userCode })
}